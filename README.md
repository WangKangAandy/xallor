...
- **Multi-desktop:** scroll the mouse wheel **down** over the desktop area to move to the next page; when on the **last** page, a **new empty** page is created **only if the current page has at least one icon** (otherwise empty pages would stack forever). Wheel **up** goes to the previous page.
...
**中文说明：** 默认桌面图标与文件夹列表由 **内置 JSON**（随扩展打包）维护；开发者改该文件即可调整「官方默认」。**多桌面** 下纵向滚轮向下切下一页；在**最后一页**且**当前页至少有一个图标**时才会再新建空白页（当前页为空则不再叠新页）。向上滚回上一页；搜索框固定在上方。持久化见 `xallor_multipage_grid_v1`，老数据会从 `xallor_grid_v1` 自动迁移。
